"""
毎年プラス + 年+100%リターンを狙う反復探索
============================================
前回の問題点を修正:
  ❌ 2020年 全戦略マイナス → COVID前ブル相場でLONG → 暴落で死
  ❌ SHORT発動なし → 厳格レジームが厳しすぎる

改良:
  ✅ ボラティリティブレーキ（BTC ATR% > 7% で新規停止）
  ✅ SHORT条件緩和（BTC < EMA200 単独でOK）
  ✅ レバ 1-2x ダイナミック
  ✅ タイトSL（-5〜-8%）で清算回避
"""
...
def run_v2(all_data, start, end, config, initial=10_000.0):
    ...
    for date in dates:
        ...
        if pd.isna(btc_e200): regime = "neutral"
        elif btc_p > btc_e200 and btc_e50 > btc_e200: regime = "bull"
        elif btc_p < btc_e200: regime = "bear"  # 緩和: EMA50下を削除
        else: regime = "neutral"

        # ボラティリティブレーキ（BTC ATR%が閾値以上なら新規停止）
        ...
        for sym in list(positions.keys()):
            ...
            if close_reason:
                ...
                trades.append({"sym": sym, "side": p["side"], "pnl": pnl, "reason": close_reason})
                del positions[sym]
            ...
            if regime == "bear" and not config["enable_short"]:
                ...
                if slots > 0:
                    ...
                    for sym, r in today_rows.items():
                        ...
                        price, ema200, rsi = r["close"], r["ema200"], r.get("rsi", 50)
    ...
    avg_annual = ((final/initial) ** (1/5) - 1) * 100 if final > 0 else 0
...
if __name__ == "__main__":
    ...
    print("📥 50銘柄×5年データ + RSI 計算中...")
    t0 = time.time()
    all_data = fetch_with_rsi(fetcher, UNIVERSE_50, "2020-01-01", "2024-12-31")
    print(f"✅ {len(all_data)}銘柄 ({time.time()-t0:.0f}秒)\n")

    # 反復: ボラブレーキ + レバ切替 + 様々な組合せ
    configs = [
        ("V01 タイトSL-5% Lev1-2 ADX25",
         dict(adx_min=25, adx_lev2=35, lev_low=1.0, lev_high=2.0,
              stop_loss_pct=0.05, trail_activate_pct=0.15, trail_giveback_pct=0.03,
              max_pos=20, rsi_short_min=70, enable_short=True, vol_brake_atr_pct=100)),
        ("V02 タイトSL-8% ボラブレーキ7% Lev1-2",
         dict(adx_min=25, adx_lev2=35, lev_low=1.0, lev_high=2.0,
              stop_loss_pct=0.08, trail_activate_pct=0.20, trail_giveback_pct=0.05,
              max_pos=20, rsi_short_min=70, enable_short=True, vol_brake_atr_pct=7)),
        ("V03 SL-5% ボラブレーキ5% Lev1-2",
         dict(adx_min=30, adx_lev2=40, lev_low=1.0, lev_high=2.0,
              stop_loss_pct=0.05, trail_activate_pct=0.12, trail_giveback_pct=0.03,
              max_pos=20, rsi_short_min=70, enable_short=True, vol_brake_atr_pct=5)),
        ("V04 Lev1-3 ADX30 SL-10%",
         dict(adx_min=30, adx_lev2=40, lev_low=1.0, lev_high=3.0,
              stop_loss_pct=0.10, trail_activate_pct=0.25, trail_giveback_pct=0.05,
              max_pos=15, rsi_short_min=70, enable_short=True, vol_brake_atr_pct=6)),
        ("V05 アグレッシブ Lev2-3 SL-6%",
         dict(adx_min=30, adx_lev2=40, lev_low=2.0, lev_high=3.0,
              stop_loss_pct=0.06, trail_activate_pct=0.20, trail_giveback_pct=0.04,
              max_pos=15, rsi_short_min=65, enable_short=True, vol_brake_atr_pct=6)),
        ("V06 保守 Lev1-1.5 SL-4% ADX35",
         dict(adx_min=35, adx_lev2=50, lev_low=1.0, lev_high=1.5,
              stop_loss_pct=0.04, trail_activate_pct=0.10, trail_giveback_pct=0.02,
              max_pos=10, rsi_short_min=75, enable_short=True, vol_brake_atr_pct=5)),
        ("V07 Lev1-2 多保有30 SL-7%",
         dict(adx_min=25, adx_lev2=35, lev_low=1.0, lev_high=2.0,
              stop_loss_pct=0.07, trail_activate_pct=0.15, trail_giveback_pct=0.04,
              max_pos=30, rsi_short_min=70, enable_short=True, vol_brake_atr_pct=6)),
        ("V08 少数精鋭5 Lev1-2 SL-5%",
         dict(adx_min=35, adx_lev2=45, lev_low=1.0, lev_high=2.0,
              stop_loss_pct=0.05, trail_activate_pct=0.15, trail_giveback_pct=0.03,
              max_pos=5, rsi_short_min=75, enable_short=True, vol_brake_atr_pct=5)),
    ]

    print(f"{'=' * 140}")
    print(f"🎯 反復探索: 毎年プラス + 年+100% + 清算回避")
    print(f"{'=' * 140}")
    print(f"{'戦略':42s} | {'2020':>7s} {'2021':>7s} {'2022':>7s} {'2023':>7s} {'2024':>7s} | "
    ...
    # 両条件達成戦略を探す
    winners_both = [(n, r) for n, r in results.items()
                    if r["all_positive"] and r["avg_annual_ret"] >= 100]
    winners_positive = [(n, r) for n, r in results.items() if r["all_positive"]]

    print(f"\n{'=' * 140}")
    if winners_both:
        print(f"🎉🎉 両条件達成（毎年プラス + 年+100%）: {len(winners_both)}戦略")
        for n, r in sorted(winners_both, key=lambda x: -x[1]["avg_annual_ret"]):
            print(f"   ✨ {n}: 年率{r['avg_annual_ret']:+.1f}% / 5年計{r['total_ret']:+.1f}% / DD{r['max_dd']:.1f}%")
    elif winners_positive:
        print(f"✅ 毎年プラス達成のみ: {len(winners_positive)}戦略")
        for n, r in sorted(winners_positive, key=lambda x: -x[1]["avg_annual_ret"]):
            print(f"   🎯 {n}: 年率{r['avg_annual_ret']:+.1f}%")
    else:

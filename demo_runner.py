...
# v2.5: multi_lookback (iter71c/d で最良効果を確認)
#   単一 25 日 → 25+45+90 日の 3 期間平均リターンで Top 選定
#   iter71d で +3.7 万円 (10 万円基準) / 2025Q1 の損失 -45% → -36% に改善
ACH_MULTI_LOOKBACK = True
ACH_MULTI_LOOKBACK_DAYS = [25, 45, 90]
...
def fetch_momentum_returns(symbols, lookback_days=90, multi_lookback_days=None):
    """各銘柄のモメンタムリターン (%) を計算

    multi_lookback_days (list[int] or None):
      None  → 単一期間 lookback_days のリターン (v2.1-2.4 互換)
      list  → 複数期間の平均リターン (v2.5 multi_lookback)
              例: [25, 45, 90] なら 25/45/90 日リターンの単純平均

    1 回の API 呼び出しで最長期間分の klines を取得し、複数期間を計算する。
    """
    returns = {}
    if multi_lookback_days:
        fetch_len = max(multi_lookback_days) + 1
    else:
        fetch_len = lookback_days + 1

    for s in symbols:
        try:
            url = (f"https://api.binance.com/api/v3/klines?symbol={s}USDT"
                   f"&interval=1d&limit={fetch_len}")
            ...
            if multi_lookback_days:
                ...
                for lb in multi_lookback_days:
                    ...
                    if start <= 0:
                        continue
                    r = (end_close - start) / start * 100
                    rets.append(r)
                    per_period[f"ret_{lb}d"] = round(r, 2)
                if not rets:
                    continue
...
def ach_update(state, btc_price_now, btc_ema200):
    ...
    if hm_refresh and not needs_rebalance:
        ...
        try:
            returns = fetch_momentum_returns(
                ACH_UNIVERSE, ACH_LOOKBACK_DAYS,
                multi_lookback_days=ACH_MULTI_LOOKBACK_DAYS if ACH_MULTI_LOOKBACK else None,
            )
            hm_data = [
    ...
    if needs_rebalance:
        ...
        # モメンタム上位2銘柄を選定 (v2.5: Top2 + dynamic_regime + multi_lookback)
...
def fresh_state():
    ...
    return {
        ...
        "ach_part": {
            ...
            "strategy": "momentum_top2_dynamic_regime_multi_lb",
...
def load_state():
    ...
    try:
        ...
        expected_cfg = {
            ...
            "multi_lookback": ACH_MULTI_LOOKBACK,
            "multi_lookback_days": list(ACH_MULTI_LOOKBACK_DAYS) if ACH_MULTI_LOOKBACK else [],
        }

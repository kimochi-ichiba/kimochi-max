...
Iter46 HTMLレポート
==========================
DDの意味を図解 + ハイブリッド12パターン比較
"""
from __future__ import annotations
import json
from pathlib import Path

DATA_PATH = Path("/Users/sanosano/projects/kimochi-max/results/iter46_hybrid.json")
OUT_PATH = Path("/Users/sanosano/projects/kimochi-max/results/iter46_report.html")
...
<style>
  * { box-sizing: border-box; }
  ...
  h3 { font-size: 17px; color: #334155; margin: 16px 0 8px 0; }
  table { width: 100%; border-collapse: collapse; margin-top: 10px;
          font-size: 13px; background: #ffffff; color: #1a1a1a; }
  th, td { padding: 10px 12px; text-align: right; border-bottom: 1px solid #e5e7eb; color: #1a1a1a; }
  ...
  .dd-case {
    padding: 18px 22px; border-radius: 10px; border: 2px solid;
  }
  ...
  .dd-case .bignum { font-size: 42px; font-weight: 800; margin: 6px 0; }
  ...
  .winner-card {
    ...
    border-radius: 12px; margin: 16px 0; border: 3px solid #10b981;
  }
  ...
  .winner-card .kpis { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr));
  ...
  .graph-box {
    background: #f8fafc; padding: 12px 18px; border-radius: 8px; margin: 10px 0;
    font-family: monospace; font-size: 13px; color: #1a1a1a;
  }
  .graph-bar { display: inline-block; height: 24px; border-radius: 4px;
               background: #ef4444; vertical-align: middle; margin-right: 6px; }
  .graph-bar.green { background: #10b981; }
  .graph-bar.yellow { background: #f59e0b; }
...
<h1>🔀 Iter46 — DDをやさしく説明＆ハイブリッド戦略の最適化</h1>
<p class="subtitle">
  「ハイブリッド50/50がすごく良いと思っている」というお声にお応えして、DDの意味から改良版までまとめました。
...
<div class="card">
  ...
  <div class="dd-visual">
    <div class="dd-case dd-bad">
      ...
      <div class="story">一番下がった時の残高。100万円が30万円に。<br>
        70万円が一時的に消えた気分。</div>
      <div style="margin-top:10px;">
        <span class="graph-bar" style="width:30%;"></span> 30万円
      </div>
      <p style="font-size:12px;">投げ売りしてしまう危険性 大。
        夜眠れない。仕事に集中できない。家庭にも影響。</p>
    </div>

    <div class="dd-case dd-mid">
      <h3>😰 DD 48%（ハイブリッド50/50）</h3>
      <div class="bignum">$52万円</div>
      <div class="story">半分近く減る。<br>
        「もうダメかも」と不安になる瞬間。</div>
      <div style="margin-top:10px;">
        <span class="graph-bar yellow" style="width:52%;"></span> 52万円
      </div>
      <p style="font-size:12px;">1-2年かけて回復することが多い。
        精神的にキツいが、長期で見れば回復する。</p>
    </div>

    <div class="dd-case dd-good">
      <h3>😊 DD 25%（低DD戦略）</h3>
      <div class="bignum">$75万円</div>
      <div class="story">75%は残る。<br>
        「まだ全然平気」と冷静でいられる。</div>
      <div style="margin-top:10px;">
        <span class="graph-bar green" style="width:75%;"></span> 75万円
      </div>
      <p style="font-size:12px;">生活に影響なし。
        夜ぐっすり眠れる。長く続けられる。</p>
    </div>
  </div>
  ...
  <div class="insight">
    ...
    <ul>
      <li><b>①「投げ売り」の誘惑</b>：「もうダメだ」と思って底値で売ってしまう →
        これが一番怖い失敗パターン。せっかくの戦略が台無しに。</li>
      <li><b>②回復に時間がかかる</b>：100万→50万に戻すには<b>2倍に増やす必要がある</b>（半値戻し問題）。
        たいてい1〜2年かかる。</li>
      <li><b>③人生への影響</b>：眠れない、家族に当たる、仕事に集中できない... お金の問題が
        生活全体に広がる。</li>
    </ul>
  </div>
</div>
...
<!-- ━━━━━━ ハイブリッドの良さ ━━━━━━ -->
<div class="card">
  <h2>🌟 なぜハイブリッド戦略が優秀なのか</h2>

  <div class="insight">
    <h3>ハイブリッドの天才的な仕組み</h3>
    <p>
      資金を「BTC保有（コツコツ派）」と「AC戦略（アクティブ派）」に分ける作戦です。<br>
      <b>どちらか一方がコケても、もう一方がカバー</b>してくれる。
    </p>
    <ul>
      <li>BTCがドカンと落ちた年（2022年） → AC戦略のフィルタで撤退して守り切る</li>
      <li>BTCが退屈な横ばいの年（2021年） → AC戦略がアルトコインで爆益を出す</li>
      <li>BTCが急上昇の年（2024年） → BTC保有分がしっかり取る</li>
    </ul>
    <p>
      「卵をひとつのカゴに盛らない」という古くからの投資格言を、<b>仮想通貨の世界で現代風に実行</b>したのがハイブリッド戦略です。
    </p>
  </div>
</div>
...
<div class="card">
  ...
  <div class="explainer">
    <h3>💡 このH11 を超おすすめする理由</h3>
    <ul>
      <li><b>清算が21件→4件に激減</b>：ACをACH（動的レバ版）に変えたことで、相場が荒い時は自動でレバレッジを下げる。清算リスクが1/5に。</li>
      <li><b>USDT 20%の現金クッション</b>：大暴落の時に「買い増しチャンス」になる現金枠。精神的な安定材料にも。</li>
      <li><b>DD 39.7%に抑制</b>：元のR10(DD 48%)から約9ポイント改善。100万円が一時的に最低$60万円で済む（元は$52万円）。</li>
      <li><b>2022年も-3.7%の軽微なダメージ</b>：BTC単体なら-64%の年に、ほぼノーダメージで通過。</li>
      <li><b>シャープレシオ1.14</b>：リスクに対してリターンが良質、と評価される水準。</li>
    </ul>
  </div>
</div>
...
<div class="kpis">
  ...
  <div class="kpi"><div class="lbl">2022年</div><div class="v">${pct(winner.yearly["2022"])}</div></div>
</div>`;
...
let tblHtml = `<tr>
  ...
  <th>年率</th><th>★DD</th><th>Sharpe</th><th>清算</th><th>最終資産</th>
</tr>`;
sorted.forEach((n, idx) => {
  ...
  if (n.startsWith("H01")) row += ' （ベースライン）';
  row += `</td>`;
...
<div class="insight">
  <h3>🟢 超安心派 → H09 or H11 or H12（DD 38-40%）</h3>
  <ul>
    <li><b>H11 BTC40%+ACH40%+USDT20%</b> (年率+54.8%、DD 39.7%、Sharpe 1.14、清算4) ← <b>一番おすすめ</b></li>
    <li><b>H09 BTC40%+ACH30%+USDT30%</b> (年率+50.5%、DD 38.1%、Sharpe 1.13、清算4)</li>
    <li><b>H12 BTC60%+ACH20%+USDT20%</b> (年率+52.1%、DD 39.7%、Sharpe 1.14、清算4)</li>
    <li>$100万円なら、一番下がっても $60万円 → 5年後 $235万円</li>
    <li>こんな人におすすめ：<b>本業が忙しい方</b>、<b>家族と住宅資金も考える方</b>、<b>初めての仮想通貨</b></li>
  </ul>
</div>

<div class="explainer">
  <h3>🔵 バランス派 → H08 or H03（DD 41-45%）</h3>
  <ul>
    <li><b>H08 BTC50%+ACH50%</b> (年率+61.0%、DD 41.0%、清算4)</li>
    <li><b>H03 BTC70%+AC30%</b> (年率+67.4%、DD 44.8%、清算21)</li>
    <li>こんな人におすすめ：<b>30〜50代で余裕資金がある方</b>、<b>中長期で最大化したい方</b></li>
  </ul>
</div>
...
<div class="explainer" style="background:#fee2e2;border-color:#fca5a5;border-left-color:#dc2626;">
  ...
  <ul>
    <li><b>H04 BTC40%+AC60% (攻め)</b> (年率+76.8%、DD 49.0%、清算21)</li>
    <li><b>H10 BTC40%+ACH30%+モメ15%+USDT15%</b> (年率+76.8%、DD 48.9%、清算4、Sharpe最高1.25)</li>
    <li>こんな人におすすめ：<b>余剰資金の少額で</b>、<b>DD 50%に耐える心構えあり</b></li>
  </ul>
</div>

<div class="insight" style="background:#0f172a;color:white;border-color:#fbbf24;">
  <h3 style="color:#fbbf24;">💡 $100万円でのシミュレーション</h3>
  <table style="color:white;">
    ...
    <tr style="background:#1e293b;">
      ...
      <td style="color:#10b981;">$1,590万円</td>
    ...
    <tr style="background:#0f172a;">
      ...
      <td style="color:#f59e0b;"><b>$60万円</b></td>
      <td style="color:white;">😊</td>
    </tr>
    <tr style="background:#1e293b;">
